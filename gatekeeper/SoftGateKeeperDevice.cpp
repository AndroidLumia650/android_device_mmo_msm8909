...
// Android 9: Helper function now writes directly to a pointer to avoid copying UniquePtrs
inline void hidl_vec2sized_buffer(gatekeeper::SizedBuffer* out, const hidl_vec<uint8_t>& vec) {
    ...
    out->buffer.reset(dummy);
    out->length = static_cast<uint32_t>(vec.size());
}
...
Return<void> SoftGateKeeperDevice::enroll(uint32_t uid,
    ...
    // Android 9: Declare SizedBuffers and populate them using the helper
    gatekeeper::SizedBuffer current_handle;
    hidl_vec2sized_buffer(&current_handle, currentPasswordHandle);
    
    gatekeeper::SizedBuffer desired_pwd;
    hidl_vec2sized_buffer(&desired_pwd, desiredPassword);
    
    gatekeeper::SizedBuffer current_pwd;
    hidl_vec2sized_buffer(&current_pwd, currentPassword);

    EnrollRequest request(uid, &current_handle, &desired_pwd, &current_pwd);
    ...
    if (response.error == ERROR_RETRY) {
        ...
        hidl_vec<uint8_t> new_handle(response.enrolled_password_handle.buffer.get(),
                                     response.enrolled_password_handle.buffer.get() +
...
Return<void> SoftGateKeeperDevice::verify(
    ...
    // Android 9: Declare SizedBuffers and populate them using the helper
    gatekeeper::SizedBuffer enrolled_handle;
    hidl_vec2sized_buffer(&enrolled_handle, enrolledPasswordHandle);
    
    gatekeeper::SizedBuffer provided_pwd;
    hidl_vec2sized_buffer(&provided_pwd, providedPassword);

    VerifyRequest request(uid, challenge, &enrolled_handle, &provided_pwd);
    ...
    if (response.error == ERROR_RETRY) {
        ...
        hidl_vec<uint8_t> auth_token(
                response.auth_token.buffer.get(),
                response.auth_token.buffer.get() + response.auth_token.length);
